...
func TestHandleCommand(t *testing.T) {
	...
	for _, test := range tests {
		t.Run(test.name, func(t *testing.T) {
			path := writeTodoFile(t, "# Todos\n\n- [ ] First\n")
			captureStdout(t, func() { HandleCommand(test.command, test.args, path) })
			if test.check == nil {
				return
			}
