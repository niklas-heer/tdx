...
sed -i.bak "s/PLACEHOLDER_DARWIN_X64/${DARWIN_X64_SHA}/" "$FORMULA"
sed -i.bak "s/PLACEHOLDER_LINUX_ARM64/${LINUX_ARM64_SHA}/" "$FORMULA"
sed -i.bak "s/PLACEHOLDER_LINUX_X64/${LINUX_X64_SHA}/" "$FORMULA"
...
sed -i.bak -E "s/sha256 \"[a-f0-9]{64}\"  # darwin-x64/sha256 \"${DARWIN_X64_SHA}\"  # darwin-x64/" "$FORMULA"
sed -i.bak -E "s/sha256 \"[a-f0-9]{64}\"  # linux-arm64/sha256 \"${LINUX_ARM64_SHA}\"  # linux-arm64/" "$FORMULA"
sed -i.bak -E "s/sha256 \"[a-f0-9]{64}\"  # linux-x64/sha256 \"${LINUX_X64_SHA}\"  # linux-x64/" "$FORMULA"
